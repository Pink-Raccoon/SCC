...
#    into float(R[inside[i]]) etc., otherwise np.linalg.norm() will give wrong results
    ...
    for i in np.arange(0,num_pix_in):
        for j in np.arange(0,num_color):
            eucl_diff[j]=np.linalg.norm([float(R[inside[i]]), float(G[inside[i]]), float(B[inside[i]])]-color_table[j,:])
...
# 6) Do plots if plot_image is set to 1
    if (plot_image):
        ...
        plt.figure(10)
        plt.subplot(221),plt.imshow(cropped_img_cl[:,:,0],cmap='Blues'),plt.axis('off'),plt.colorbar(),plt.title('Blue Channel')
        ...
        plt.figure(11)
        plt.subplot(221),plt.imshow(cropped_img_cl[:,:,1],cmap='Greens'),plt.axis('off'),plt.colorbar(),plt.title('Green Channel')
        ...
        plt.figure(12)
        plt.subplot(221),plt.imshow(cropped_img_cl[:,:,1],cmap='Reds'),plt.axis('off'),plt.colorbar(),plt.title('Red Channel')
